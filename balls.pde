...
class BALLS { // class for manipulaitng and displaying points
 ...
 int[] m = new int [maxnv];         // delay before resetting color
  ...
  BALLS(BALLS copy) {copyFrom(copy);}
  ...
  void initPointsOnGrid(int nb, float w, float r, color c) { // number of BALLS in each dimension, distance between them 
    ...
    for (int i=0; i<nb; i++) 
      for (int j=0; j<nb; j++) 
        for (int k=0; k<nb; k++)
          //if(k == 0) 
           // addBall(P(d*i-w/2+dd,d*j-w/2+dd,d*k-w/2+dd-50),V(0.1,0,0.5f),r*2,c);
          //else
           // addBall(P(d*i-w/2+dd,d*j-w/2+dd,d*k-w/2+dd+20),V(0,0,1),r*2,c);
  ...
  void bounceBalls(float w) {
    for (int v=0; v<nv; v++) {
      if(G[v].x<-w/2+r[v] || G[v].x>w/2-r[v]) {c[v]=red; m[v]=del; if(individual) stop=true; collisions++;}
      if(G[v].y<-w/2+r[v] || G[v].y>w/2-r[v]) {c[v]=red; m[v]=del; if(individual) stop=true; collisions++;}
      if(G[v].z<-w/2+r[v] || G[v].z>w/2-r[v]) {c[v]=red; m[v]=del; if(individual) stop=true; collisions++;}
      }
